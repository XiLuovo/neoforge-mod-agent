...
class ModSpec:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "machines": [_serialize(machine) for machine in self.machines],
            "entities": [_serialize(entity) for entity in self.entities],
            "dimensions": [_serialize(dimension) for dimension in self.dimensions],
            "biomes": [_serialize(biome) for biome in self.biomes],
            "world_features": [_serialize(feature) for feature in self.world_features],
            "structures": [_serialize(structure) for structure in self.structures],
            "loot_pools": [_serialize(pool) for pool in self.loot_pools],
            "java_extensions": [_serialize(extension) for extension in self.java_extensions],
            "ores": [_serialize(ore) for ore in self.ores],
            "foods": [_serialize(food) for food in self.foods],
            "swords": [_serialize(sword) for sword in self.swords],
            "tools": [_serialize(tool) for tool in self.tools],
            "armors": [_serialize(armor) for armor in self.armors],
            "recipes": [_serialize(recipe) for recipe in self.recipes],
            "progressions": [_serialize(progression) for progression in self.progressions],
            "balance_plans": [_serialize(plan) for plan in self.balance_plans],
            "quests": [_serialize(quest) for quest in self.quests],
            "features": [_serialize(feature) for feature in self.iter_features()],
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ModSpec":
        ...
        if features:
            ...
            typed_machines: list[dict[str, Any]] = []
            typed_entities: list[dict[str, Any]] = []
            typed_dimensions: list[dict[str, Any]] = []
            typed_biomes: list[dict[str, Any]] = []
            typed_world_features: list[dict[str, Any]] = []
            typed_structures: list[dict[str, Any]] = []
            typed_loot_pools: list[dict[str, Any]] = []
            typed_java_extensions: list[dict[str, Any]] = []
            typed_ores: list[dict[str, Any]] = []
            typed_foods: list[dict[str, Any]] = []
            typed_swords: list[dict[str, Any]] = []
            typed_tools: list[dict[str, Any]] = []
            typed_armors: list[dict[str, Any]] = []
            typed_recipes: list[dict[str, Any]] = []
            typed_progressions: list[dict[str, Any]] = []
            typed_balance_plans: list[dict[str, Any]] = []
            typed_quests: list[dict[str, Any]] = []
            for feature in features:
                feature_type = str(feature.get("type", "")).lower()
                if feature_type == "item":
                    typed_items.append(feature)
                elif feature_type == "block":
                    typed_blocks.append(feature)
                elif feature_type == "machine":
                    typed_machines.append(feature)
                elif feature_type == "entity":
                    typed_entities.append(feature)
                elif feature_type == "dimension":
                    typed_dimensions.append(feature)
                elif feature_type == "biome":
                    typed_biomes.append(feature)
                elif feature_type == "world_feature":
                    typed_world_features.append(feature)
                elif feature_type == "structure":
                    typed_structures.append(feature)
                elif feature_type == "loot_pool":
                    typed_loot_pools.append(feature)
                elif feature_type == "java_extension":
                    typed_java_extensions.append(feature)
                elif feature_type == "ore":
                    typed_ores.append(feature)
                elif feature_type == "food":
                    typed_foods.append(feature)
                elif feature_type == "sword":
                    typed_swords.append(feature)
                elif feature_type == "tool":
                    typed_tools.append(feature)
                elif feature_type == "armor":
                    typed_armors.append(feature)
                elif feature_type == "recipe":
                    typed_recipes.append(feature)
                elif feature_type == "progression":
                    typed_progressions.append(feature)
                elif feature_type == "balance_plan":
                    typed_balance_plans.append(feature)
                elif feature_type == "quest":
                    typed_quests.append(feature)
            seen_progression_ids = {
                str(progression.get("id", progression.get("identifier", "")))
                for progression in typed_progressions
            }
            for progression in data.get("progressions", []):
                progression_id = str(progression.get("id", progression.get("identifier", "")))
                if progression_id not in seen_progression_ids:
                    typed_progressions.append(progression)
                    seen_progression_ids.add(progression_id)
            seen_balance_plan_ids = {
                str(plan.get("id", plan.get("identifier", "")))
                for plan in typed_balance_plans
            }
            for plan in data.get("balance_plans", []):
                plan_id = str(plan.get("id", plan.get("identifier", "")))
                if plan_id not in seen_balance_plan_ids:
                    typed_balance_plans.append(plan)
                    seen_balance_plan_ids.add(plan_id)
            seen_quest_ids = {
                str(quest.get("id", quest.get("identifier", "")))
                for quest in typed_quests
            }
            for quest in data.get("quests", []):
                quest_id = str(quest.get("id", quest.get("identifier", "")))
                if quest_id not in seen_quest_ids:
                    typed_quests.append(quest)
                    seen_quest_ids.add(quest_id)
        else:
            typed_items = list(data.get("items", []))
            typed_blocks = list(data.get("blocks", []))
            typed_machines = list(data.get("machines", []))
            typed_entities = list(data.get("entities", []))
            typed_dimensions = list(data.get("dimensions", []))
            typed_biomes = list(data.get("biomes", []))
            typed_world_features = list(data.get("world_features", []))
            typed_structures = list(data.get("structures", []))
            typed_loot_pools = list(data.get("loot_pools", []))
            typed_java_extensions = list(data.get("java_extensions", []))
            typed_ores = list(data.get("ores", []))
            typed_foods = list(data.get("foods", []))
            typed_swords = list(data.get("swords", []))
            typed_tools = list(data.get("tools", []))
            typed_armors = list(data.get("armors", []))
            typed_recipes = list(data.get("recipes", []))
            typed_progressions = list(data.get("progressions", []))
            typed_balance_plans = list(data.get("balance_plans", []))
            typed_quests = list(data.get("quests", []))
        ...
        return cls(
            ...
            items=[_item_from_dict(entry) for entry in typed_items],
            blocks=[_block_from_dict(entry) for entry in typed_blocks],
            machines=[_machine_from_dict(entry) for entry in typed_machines],
            entities=[_entity_from_dict(entry) for entry in typed_entities],
            dimensions=[_dimension_from_dict(entry) for entry in typed_dimensions],
            biomes=[_biome_from_dict(entry) for entry in typed_biomes],
            world_features=[_world_feature_from_dict(entry) for entry in typed_world_features],
            structures=[_structure_from_dict(entry) for entry in typed_structures],
            loot_pools=[_loot_pool_from_dict(entry) for entry in typed_loot_pools],
            java_extensions=[_java_extension_from_dict(entry) for entry in typed_java_extensions],
            ores=[_ore_from_dict(entry) for entry in typed_ores],
            foods=[_food_from_dict(entry) for entry in typed_foods],
            swords=[_sword_from_dict(entry) for entry in typed_swords],
            tools=[_tool_from_dict(entry) for entry in typed_tools],
            armors=[_armor_from_dict(entry) for entry in typed_armors],
            recipes=[_recipe_from_dict(entry) for entry in typed_recipes],
            progressions=[_progression_from_dict(entry) for entry in typed_progressions],
            balance_plans=[_balance_plan_from_dict(entry) for entry in typed_balance_plans],
            quests=[_quest_from_dict(entry) for entry in typed_quests],
...
def _recipe_from_dict(data: dict[str, Any]) -> RecipeSpec:
    return RecipeSpec(
        ...
        keys={str(key): str(value) for key, value in data.get("keys", {}).items()},
