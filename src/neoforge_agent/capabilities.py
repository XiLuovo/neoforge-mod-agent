...
class CapabilityCatalog:
        ...
        return [
            ...
            CapabilitySection(
                ...
                capabilities=[
                    ...
                    Capability("agent_generate", "Agent Generate", "stable", "Run planner, reviewer, executor, auditor, and repair roles for new workspace generation with ModSpec-first routing; Direct Code Lane remains an experimental opt-in path.", ["agent generate --code-lane hybrid"], [".agent/agent-run.json", ".agent/agent-run.md", ".agent/agent-decisions.md", ".agent/prompt-trace.json"]),
                    Capability("agent_modify", "Agent Modify", "stable", "Run multi-role orchestration for incremental workspace modification with ModSpec-first routing; Direct Code Lane remains an experimental opt-in path.", ["agent modify --code-lane hybrid"], [".agent/agent-run.json", ".agent/agent-run.md", ".agent/agent-decisions.md", ".agent/prompt-trace.json"]),
                    Capability("direct_code_lane", "Direct Code Lane", "experimental", "Apply reviewed structured JSON source patches inside generated workspaces as an experimental escape hatch; stable patterns must be folded back into ModSpec, DSL, generator, audit, and tests.", ["agent generate --code-lane direct", "agent modify --code-lane direct"], [".agent/direct-code-plan.json", ".agent/direct-code-review.json", ".agent/direct-code-diff.md", ".agent/direct-code-report.json", ".agent/direct-code-rollback-report.json"]),
                    ...
                    Capability("benchmark_report_page", "Benchmark Report Page", "stable", "Aggregate model A/B, mock/real provider status, failure types, repair rate, build pass rate, and separately documented manual runtime evidence into one static HTML benchmark page.", ["benchmark-report"], ["workspace/benchmark-runs/<run-id>/.agent/benchmark-report.html"]),
                    Capability("evidence_chain_report", "Layered Evidence Chain Report", "stable", "Aggregate Stable ModSpec, Behavior DSL, and controlled patch-agent proof into one report with success rates, failure samples, recovery rates, generated file counts, and manual runtime evidence scope.", ["evidence-chain-report"], ["workspace/evidence-chain-runs/<run-id>/.agent/evidence-chain-report.json", "workspace/evidence-chain-runs/<run-id>/.agent/evidence-chain-report.md"]),
                    ...
                    Capability("portfolio_release_package", "Portfolio Release Package", "stable", "Provide a V5.0 one-command demo script, README entry, architecture diagrams, screenshot checklist, and curated demo cases for public presentation.", ["scripts/v5_portfolio_demo.ps1"], ["README.md", "docs/portfolio-release.md", "docs/总览/architecture.md", "docs/发布与展示/demo-cases.md", "docs/发布与展示/screenshots.md"]),
    ...
    def _limitations(self) -> list[str]:
        return [
            ...
            "Direct Code Lane is a structured patch lane for generated workspaces only; it is not an unbounded coding agent and cannot modify this tool's source tree, .git, Gradle wrapper jars, build outputs, or paths outside the workspace; acceptance still depends on audit/build gates and rollback evidence.",
