...
class DirectCodeAgent:
    ...
    def apply_plan(
        ...
        *,
        ...
        artifacts = direct_code_artifacts(workspace, self.config)
        review = self.review_plan(workspace, plan)
        write_json(artifacts["plan_json"], plan.to_dict())
        write_text(artifacts["plan_md"], render_direct_code_plan_md(plan))
        write_json(artifacts["review_json"], review.to_dict())
        ...
        if review.approved:
            ...
            diff_text = render_direct_code_diff(before_text_by_path, after_text_by_path)
        else:
            errors.extend(review.errors)

        write_text(artifacts["diff_md"], diff_text or "# Direct Code Diff\n\nNo changes were applied.\n")
...
def render_direct_code_plan_md(plan: DirectCodePlan) -> str:
    lines = [
        "# Direct Code Plan",
        "",
        f"Mode: `{plan.mode}`",
...
def render_direct_code_diff(before_by_path: dict[str, str], after_by_path: dict[str, str]) -> str:
