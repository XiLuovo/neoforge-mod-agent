...
from .llm_planner import LLMPlanningError, plan_with_decomposed_llm, plan_with_llm, write_planner_artifacts
...
def build_parser() -> argparse.ArgumentParser:
    ...
    eval_parser.add_argument("--require-llm", action="store_true", help="Fail eval cases instead of falling back when an LLM-backed planner cannot produce real output.")
    ...
    real_llm_stability_parser.add_argument("--planner", choices=["llm", "decomposed"], default="decomposed", help="Planner lane to test; decomposed is the default real-LLM path.")
    ...
    benchmark_report_parser.add_argument("--runtime-evidence", help="Optional JSON or Markdown file with documented Minecraft runtime evidence.")
    ...
    _hide_subparser_from_help(subparsers, "harvest-report")
    ...
    _hide_subparser_from_help(agent_subparsers, "lab-generate")
    ...
    _refresh_subparser_metavar(subparsers)
    _refresh_subparser_metavar(agent_subparsers)

    return parser


def _hide_subparser_from_help(subparsers: argparse._SubParsersAction, command_name: str) -> None:
    subparsers._choices_actions = [
        action for action in subparsers._choices_actions if action.dest != command_name
    ]


def _refresh_subparser_metavar(subparsers: argparse._SubParsersAction) -> None:
    public_choices = [action.dest for action in subparsers._choices_actions]
    subparsers.metavar = "{" + ",".join(public_choices) + "}"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "print-schema":
        print(json.dumps(get_modspec_schema(), indent=2, ensure_ascii=True))
        return 0
    ...
    try:
        planner_resolution = _resolve_spec_from_prompt(
