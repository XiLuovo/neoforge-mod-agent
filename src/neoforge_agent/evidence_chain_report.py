...
from . import neoforge_modify_workflow as neoforge_modify_workflow_module
from .agent_orchestrator import AgentOrchestrator
from .auditor import WorkspaceAuditor
from .benchmark_report import BenchmarkReportRunner
from .config import AppConfig
from .manual_runtime_evidence import summarize_manual_runtime_evidence
...
EVIDENCE_CHAIN_SCOPE = (
    "Evidence-chain success aggregates workspace-level audit/build gates, generated reports, repair evidence, "
    "and any documented manual runtime matrix entries. It does not claim Minecraft client/server runtime "
    "acceptance unless a manual runtime evidence case is explicitly present."
)
...
class EvidenceChainReportRunner:
    ...
    def _run_stable_layer(
        ...
        *,
        ...
        manual_runtime_evidence = summarize_manual_runtime_evidence(benchmark.runtime_cases).to_dict()
        runtime_validation = {
            "manual_runtime_evidence": manual_runtime_evidence,
            "runtime_cases_total": manual_runtime_evidence["runtime_cases_total"],
            "runtime_pass_rate": manual_runtime_evidence["runtime_pass_rate"],
            "runtime_cases": manual_runtime_evidence["runtime_cases"],
        ...
        return EvidenceLayerResult(
            ...
            notes=[
                "Manual runtime evidence is only counted when documented Minecraft runtime matrix entries are present.",
            ],
        )
    ...
    def _run_behavior_layer(self, scoped_config: AppConfig, root_dir: Path, run_id: str) -> EvidenceLayerResult:
        ...
        return EvidenceLayerResult(
            ...
            notes=[
                "Behavior evidence is generated behavior reports, audit results, and manual test checklists; the checklist is not proof of completed in-game runtime testing by itself.",
            ],
        )
    ...
    def _run_patch_agent_layer(self, scoped_config: AppConfig, root_dir: Path, run_id: str) -> EvidenceLayerResult:
        ...
        return EvidenceLayerResult(
            ...
            notes=[
                "Patch-agent evidence is audit/build gate output plus rollback and repair-loop reports, not automatic Minecraft runtime acceptance.",
            ],
        )
    ...
    def _run_patch_case(
        ...
        *,
        ...
        if failure_mode:
            ...
            with patch.object(neoforge_modify_workflow_module, "WorkspaceModifier", _FailingWorkspaceModifier):
    ...
    def _metrics(self, layers: list[EvidenceLayerResult]) -> dict[str, Any]:
        ...
        for layer in layers:
            ...
            manual_runtime = runtime.get("manual_runtime_evidence")
            if isinstance(manual_runtime, dict):
                runtime_validation_total += int(manual_runtime.get("runtime_cases_total", 0) or 0)
                runtime_passed_total += int(manual_runtime.get("runtime_passed_count", 0) or 0)
        runtime_pass_rate = _rate(runtime_passed_total, runtime_validation_total)
        return {
            ...
            "runtime_validation_pass_rate": runtime_pass_rate,
            "manual_runtime_cases_total": runtime_validation_total,
            "manual_runtime_passed_count": runtime_passed_total,
            "manual_runtime_pass_rate": runtime_pass_rate,
            "evidence_scope": EVIDENCE_CHAIN_SCOPE,
    ...
    def _render_markdown(self, result: EvidenceChainReportResult) -> str:
        ...
        for key in (
            ...
            "manual_runtime_cases_total",
            "manual_runtime_pass_rate",
        ):
        ...
        for layer in result.layers:
            ...
            if runtime_validation:
                lines.append("- evidence details (`runtime_validation` legacy field):")
                for key, value in runtime_validation.items():
                    if key == "manual_runtime_evidence" and isinstance(value, dict):
                        lines.append(
                            "  - manual_runtime_evidence: "
                            f"`{value.get('runtime_cases_total', 0)}` case(s), "
                            f"pass_rate=`{value.get('runtime_pass_rate')}`"
                        )
                        continue
