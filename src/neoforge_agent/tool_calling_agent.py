...
class StructuredPatchApplier:
    ...
    def apply(self, workspace: Path, payload: dict[str, Any]) -> StructuredPatchResult:
        ...
        artifacts = self.evidence_writer.structured_patch_artifacts(workspace)
        ...
        self.evidence_writer.write_structured_patch_plan(
            artifacts,
        ...
        diff_text = self.evidence_writer.render_structured_patch_diff(before_by_path, after_by_path)
        success = not errors
        report = {
            "version": "1.0",
        ...
        self.evidence_writer.write_structured_patch_report(
            artifacts,
            diff_text=diff_text,
...
class ToolCallingRepairAgent:
    ...
    def _retrieve_rag(self, workspace: Path, args: dict[str, Any], *, state: dict[str, Any]) -> dict[str, Any]:
        ...
        paths = self.evidence_writer.repair_rag_context_paths(workspace)
        ...
        self.evidence_writer.write_repair_rag_observation(workspace, observation)
        return observation
    ...
    def _write_repair_report(self, result: ToolCallingRepairResult) -> None:
        self.evidence_writer.write_tool_calling_repair_report(result)
...
def render_rag_markdown(observation: dict[str, Any]) -> str:
    return AgentEvidenceWriter(AppConfig.default()).render_repair_rag_observation_md(observation)
