...
class ToolManifestRunner:
        ...
        return [
            ToolContract(
                ...
                safety_boundaries=[
                    ...
                    "Direct Code plans are reviewed for path ownership, operation type, risky tokens, snapshots, and build/audit gates before the run is treated as accepted.",
                ],
            ...
            ToolContract(
                ...
                safety_boundaries=[
                    ...
                    "Repair iterations are bounded by max_iterations and stay inside generated managed files unless a reviewed Direct Code plan is selected.",
                ],
                side_effects=["Creates or overwrites a generated workspace when overwrite is true."],
                maps_to=["Function Calling tool", "future MCP tool"],
            ),
            ToolContract(
                ...
                safety_boundaries=[
                    "Only generated workspace files under approved Direct Code roots are eligible for mutation.",
                    "Patch plans are structured JSON, reviewed before execution, snapshotted before writes, and recorded with rollback evidence.",
            ...
            ToolContract(
                ...
                title="Agent Bench",
                category="evaluation",
                description="Run eval, failure repair, build/audit, RAG, and benchmark aggregation as one coding-agent benchmark command.",
            ...
            ToolContract(
                name="free_code_lab_generate",
                title="Free-Code Lab Generate",
                category="experimentation",
                description="Copy an existing generated workspace into an isolated lab run, apply structured experimental source/resource patches, and write harvest-candidate evidence.",
                cli_mapping="agent lab-generate",
                input_schema=_object_schema(
                    required=["request", "from_workspace"],
                    properties={
                        "request": _string("Natural language generate gap or experimental feature request."),
                        "from_workspace": _string("Source generated workspace path or workspace name to copy into the lab run."),
                        "run_name": _string("Stable run folder name under workspace/free-code-lab-runs."),
                        "llm_provider": _enum(["mock", "openai-compatible"], "LLM provider used to propose the experimental plan."),
                        "run_build": _boolean("Run Gradle build inside the copied lab workspace."),
                    },
                ),
                output_artifacts=[
                    "workspace/free-code-lab-runs/<run-id>/.agent/free-code-plan.json",
                    "workspace/free-code-lab-runs/<run-id>/.agent/free-code-diff.md",
                    "workspace/free-code-lab-runs/<run-id>/.agent/free-code-report.json",
                    "workspace/free-code-lab-runs/<run-id>/.agent/manual-runtime-checklist.md",
                    "workspace/free-code-lab-runs/<run-id>/.agent/harvest-candidate.json",
                ],
                safety_boundaries=[
                    "Copies the source workspace and applies changes only inside workspace/free-code-lab-runs/<run-id>/workspace.",
                    "Rejects absolute paths, traversal, .git, Gradle wrapper jars, build outputs, binary artifacts, and dangerous Java tokens.",
                    "Does not modify neoforge-mod-agent source code or update stable generator templates.",
                    "Requires harvest review and manual runtime checklist evidence before a lab sample can be promoted.",
                ],
                side_effects=["Creates an isolated Free-Code Lab run folder under the configured workspace root; existing run names are rejected instead of overwritten."],
                maps_to=["Function Calling tool", "future MCP tool"],
            ),
            ToolContract(
                name="harvest_report",
                ...
                category="reporting",
                description="Aggregate Free-Code Lab harvest candidates and summarize readiness, blockers, generate gaps, and harvest directions.",
                cli_mapping="harvest-report",
                input_schema=_object_schema(
                    properties={
                        "run_name": _string("Stable run folder name under workspace/harvest-runs."),
                    },
                ),
                output_artifacts=[
                    "workspace/harvest-runs/<run-id>/.agent/harvest-report.json",
                    "workspace/harvest-runs/<run-id>/.agent/harvest-report.md",
                ],
                safety_boundaries=[
                    "Aggregates existing local harvest-candidate.json files without modifying generator code.",
                    "Does not mark a sample as harvested automatically; promotion remains a separate implementation and test step.",
                ],
                side_effects=["Writes a harvest report under workspace/harvest-runs."],
                maps_to=["Function Calling tool", "future MCP tool/resource"],
            ),
            ToolContract(
            ...
            ToolContract(
                ...
                category="reporting",
                description="Aggregate stable ModSpec, Behavior DSL, and controlled patch-agent proof into one layered evidence report.",
    ...
    def _limitations(self) -> list[str]:
        return [
            ...
            "Free-Code Lab explores generate gaps in copied workspaces only; it records harvest candidates but never writes successful experiments back into generator code automatically.",
        ]
