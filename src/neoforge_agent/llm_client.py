...
def inspect_llm_provider_config(provider: str = "openai-compatible") -> LLMProviderConfig:
    ...
    response_format, response_format_source = _env_first(
        ["NEOFORGE_AGENT_LLM_RESPONSE_FORMAT", "OPENAI_RESPONSE_FORMAT"],
        default="json_object",
    )
    ...
    response_format = (response_format or "json_object").strip().lower()
    if response_format in {"", "default"}:
        response_format = "json_object"
    if response_format not in {"json_object", "none"}:
        warnings.append(f"Invalid LLM response format `{response_format}`; using json_object.")
        response_format = "json_object"
    ...
    return LLMProviderConfig(
        ...
        env_sources={
            ...
            "response_format": response_format_source,
...
class MockLLMClient:
    ...
    def complete_json(self, system_prompt: str, user_prompt: str) -> LLMCompletion:
        ...
        if "TOOL_CALLING_REPAIR_AGENT" in system_prompt:
            ...
            return self._completion(system_prompt, user_prompt, payload)
        if "DECOMPOSED_MODIFY_PLAN_V2" in system_prompt:
            payload = self._mock_decomposed_modify_plan_payload(user_prompt)
            return self._completion(system_prompt, user_prompt, payload)
        if "DECOMPOSED_MODIFY_FEATURE_JSON_V2" in system_prompt:
            payload = self._mock_decomposed_modify_feature_json_payload(user_prompt)
            return self._completion(system_prompt, user_prompt, payload)
    ...
    def _mock_decomposed_modify_plan_payload(self, user_prompt: str) -> dict:
        existing_context = self._json_object_after_marker(user_prompt, "Existing workspace compact context JSON:")
        change_request = user_prompt
        if "Change Request:" in user_prompt:
            change_request = user_prompt.split("Change Request:", 1)[1]
        source = self._mock_modify_payload(user_prompt)
        existing_ids = {
            str(feature.get("id", ""))
            for feature in existing_context.get("features", [])
            ...
            features.append(
                {
                    "type": feature_type,
                    "id": identifier,
                    "operation": "update" if identifier in existing_ids else "add",
                    "intent": f"Mock decomposed modify planner selected {feature_type} '{identifier}'.",
                    "depends_on": self._mock_decomposed_dependencies(feature),
        ...
        return {
            "mod_id": existing_context.get("mod_id", source.get("mod_id", "ruby_mod")),
            "mod_name": existing_context.get("mod_name", source.get("mod_name", "Ruby Mod")),
            "package": existing_context.get("package", source.get("package", "com.generated.ruby_mod")),
            "version": existing_context.get("version", source.get("version", "0.1.0")),
            "description": f"Mock decomposed modify plan for: {change_request.strip()}",
            "features": features,
        }

    def _mock_decomposed_modify_feature_json_payload(self, user_prompt: str) -> dict:
        target = self._json_object_after_marker(user_prompt, "Target modify feature plan item JSON:")
        fields = dict(target.get("fields")) if isinstance(target.get("fields"), dict) else {}
        feature_type = str(target.get("type", fields.get("type", "item")))
        identifier = str(target.get("id", fields.get("id", "generated_feature")))
        display_name = str(target.get("display_name_en_us", fields.get("display_name_en_us", identifier.replace("_", " ").title())))
    ...
    def _mock_modify_payload(self, user_prompt: str) -> dict:
        ...
        if "progression" in prompt and any(token in prompt for token in ("mastery", "milestone", "final")):
            features.append(
                {
                    "type": "progression",
                    "id": "ruby_progression",
                    "title": "Ruby Progression",
                    "summary": "Updated ruby progression with a final mastery milestone.",
                    "end_stage": "master_ruby_progression",
                    "stages": [
                        {
                            "id": "master_ruby_progression",
                            "type": "milestone",
                            "title": "Master Ruby Progression",
                            "description": "Final mastery milestone after entering the ruby realm.",
                            "requires": ["ruby_sword", "ruby_pickaxe"],
                            "provides": ["ruby_mastery"],
                            "unlocks": [],
                    ...
                    "links": [
                        {
                            "from": "enter_ruby_realm",
                            "to": "master_ruby_progression",
                            "trigger": "enter_realm",
                            "requirement": "Ruby realm entered.",
                        }
                    ],
                }
            )
    ...
    def complete_json(self, system_prompt: str, user_prompt: str) -> LLMCompletion:
        ...
        for attempt in range(1, attempts + 1):
            ...
            try:
                ...
                raw_text, text_diagnostics = _openai_compatible_choice_text(choice)
                attempt_summary = _provider_attempt_summary(
                    attempt=attempt,
                    success=True,
                    latency_ms=int((time.perf_counter() - attempt_started_at) * 1000),
                    status_code=status_code,
                )
                attempt_summary.update(text_diagnostics)
...
def _openai_compatible_choice_text(choice: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    if message:
        ...
        diagnostics["message_content_type"] = type(content).__name__ if content_present else "missing"
        text = _coerce_provider_text(content)
        if text:
            diagnostics["response_text_source"] = "message.content"
            diagnostics["response_text_chars"] = len(text)
            return text, diagnostics

        for key in ("text", "output_text"):
            text = _coerce_provider_text(message.get(key))
            if text:
                diagnostics["response_text_source"] = f"message.{key}"
                diagnostics["response_text_chars"] = len(text)
                return text, diagnostics

        diagnostics["message_content_empty"] = content_present
        diagnostics["message_has_reasoning_content"] = bool(
            message.get("reasoning_content") or message.get("reasoning") or message.get("reasoning_text")
        )
        diagnostics["message_has_tool_calls"] = bool(message.get("tool_calls"))

    text = _coerce_provider_text(choice.get("text"))
    if text:
        diagnostics["response_text_source"] = "choice.text"
        diagnostics["response_text_chars"] = len(text)
        return text, diagnostics
...
def _coerce_provider_text(value: Any) -> str:
    if value is None:
        return ""
