...
$learningNotesDirName = -join ([char[]](0x5b66, 0x4e60, 0x7b14, 0x8bb0))
$historyArchiveDirName = -join ([char[]](0x5386, 0x53f2, 0x6863, 0x6848))
$privateDocRelativePaths = @(
    "docs\$learningNotesDirName",
    "docs\$historyArchiveDirName"
)
...
function Remove-ReleasePath {
    ...
    if (-not ($resolvedTarget -eq $resolvedStage -or $resolvedTarget.StartsWith("$resolvedStage\"))) {
        throw "Refusing to remove path outside release staging: $resolvedTarget"
    }

    Remove-Item -LiteralPath $resolvedTarget -Recurse -Force
}

function Convert-ToPublicSafeText {
    param(
    ...
    $textNames = @(".env.example", ".gitignore")
    $repoRootVariants = @($repoRoot)
    foreach ($slashCount in @(2, 4, 8, 16)) {
        $repoRootVariants += $repoRoot.Replace("\", ("\" * $slashCount))
    }
    ...
    Get-ChildItem -LiteralPath $RootPath -File -Recurse -Force |
        ...
        ForEach-Object {
            ...
            foreach ($variant in $repoRootVariants) {
                $content = $content.Replace($variant, "<repo>")
            }
...
$publicExcludedRelativePaths = @(
    "AGENTS.md",
    "CODING_AGENT_UPGRADE_PLAN.md"
) + $privateDocRelativePaths

foreach ($path in $publicExcludedRelativePaths) {
    Remove-ReleasePath -RelativePath $path
}
...
$forbiddenRoots = @("workspace", ".tmp", "dist", ".gradle-user-home", ".gradle-default-user-home", ".playwright-mcp", ".codex") + $privateDocRelativePaths
$violations = @()
...
$manifest = [ordered]@{
    ...
    source_root = "<repo>"
    stage_path = Join-Path $OutputRoot $ReleaseName
    zip_path = if ($NoZip) { $null } else { Join-Path $OutputRoot "$ReleaseName.zip" }
...
Convert-ToPublicSafeText -RootPath $stagePath
