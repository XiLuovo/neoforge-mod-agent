...
if ([string]::IsNullOrWhiteSpace($RunName)) {
    $RunName = "rc1-demo-$((Get-Date).ToString('yyyyMMdd-HHmmss'))"
}
...
Write-Host "== NeoForge Mod Agent RC1 Demo =="
...
Invoke-AgentCli -Arguments @(
    "agent", "develop",
    "Create a ruby mod with a ruby item, ruby block and ruby ore.",
    "--planner", $Planner,
...
Invoke-AgentCli -Arguments @(
    ...
    "--goal", "Fix audit failures using safe structured patches.",
    "--planner", $Planner,
    "--llm-provider", $Provider,
    "--max-iterations", "$MaxIterations",
    $BuildFlag,
...
Write-Host ""
Write-Host "Suggested next step: open agent-run.md, then compare tool-call-trace.json with the benchmark report."
