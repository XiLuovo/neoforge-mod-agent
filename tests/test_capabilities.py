...
class CapabilityCatalogTests(unittest.TestCase):
    def test_capability_catalog_writes_reports_and_includes_core_sections(self) -> None:
        with tempfile.TemporaryDirectory(prefix="neoforge-agent-", dir=TMP_ROOT) as tmp:
            ...
            self.assertNotIn("free_code_lab", capability_ids)
            self.assertNotIn("capability_harvest_report", capability_ids)
            self.assertNotIn("capability_harvest_loop", capability_ids)
            self.assertNotIn("machine_gui_harvest_target", capability_ids)
            self.assertNotIn("free_code_lab_safety_gate", capability_ids)

            by_id = {capability.identifier: capability for section in result.sections for capability in section.capabilities}
            self.assertEqual(by_id["direct_code_lane"].status, "experimental")
            self.assertIn("experimental escape hatch", by_id["direct_code_lane"].summary)
            self.assertIn("experimental opt-in", by_id["agent_generate"].summary)
            self.assertIn("experimental opt-in", by_id["agent_modify"].summary)
