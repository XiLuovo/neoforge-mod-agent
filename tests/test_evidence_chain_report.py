...
class EvidenceChainReportTests(unittest.TestCase):
    def test_evidence_chain_report_aggregates_all_layers(self) -> None:
        with tempfile.TemporaryDirectory(prefix="neoforge-agent-", dir=TMP_ROOT) as tmp:
            ...
            self.assertEqual(result.metrics["runtime_validation_pass_rate"], 0.0)
            self.assertEqual(result.metrics["manual_runtime_cases_total"], 0)
            self.assertEqual(result.metrics["manual_runtime_pass_rate"], 0.0)
            self.assertIn("workspace-level audit/build gates", result.metrics["evidence_scope"])
            ...
            stable_layer = next(layer for layer in result.layers if layer.identifier == "stable")
            manual_runtime = stable_layer.runtime_validation["manual_runtime_evidence"]
            self.assertEqual(manual_runtime["schema_version"], "manual-runtime-evidence/v1")
            self.assertEqual(manual_runtime["evidence_kind"], "manual_minecraft_runtime")
            self.assertEqual(manual_runtime["runtime_cases_total"], 0)

            report = result.evidence_chain_report_md_path.read_text(encoding="utf-8")
            self.assertIn("Evidence Boundary", report)
            self.assertIn("does not claim Minecraft client/server runtime acceptance", report)
