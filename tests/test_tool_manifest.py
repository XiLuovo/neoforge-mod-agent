...
class ToolManifestTests(unittest.TestCase):
    def test_tool_manifest_writes_reports_and_core_tool_schemas(self) -> None:
        with tempfile.TemporaryDirectory(prefix="neoforge-agent-", dir=TMP_ROOT) as tmp:
            ...
            self.assertNotIn("free_code_lab_generate", tool_names)
            self.assertNotIn("harvest_report", tool_names)
            ...
            self.assertIn("experimental", agent_generate.input_schema["properties"]["code_lane"]["description"])
            ...
            self.assertTrue(any("experimental opt-in lane" in boundary for boundary in agent_generate.safety_boundaries))
            ...
            self.assertIn("experimental", agent_modify.input_schema["properties"]["code_lane"]["description"])
            self.assertIn(".agent/direct-code-review.json", agent_modify.output_artifacts)
            self.assertTrue(any("experimental Direct Code lane" in boundary for boundary in agent_modify.safety_boundaries))
