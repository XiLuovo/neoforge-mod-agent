...
from neoforge_agent import AppConfig, ModProjectPlanner, PlannerResolution, RequestOverrides
from neoforge_agent.cli import _collect_agent_run_metrics, _resolve_spec_from_prompt, build_parser


class CliParserTests(unittest.TestCase):
    def test_resolve_spec_from_prompt_returns_named_planner_resolution(self) -> None:
        config = AppConfig.default()
        args = build_parser().parse_args(["plan", "Create a ruby item.", "--planner", "rules"])

        resolution = _resolve_spec_from_prompt(
    ...
    def test_top_level_help_mentions_eval_and_agent(self) -> None:
        ...
        self.assertNotIn("harvest-report", help_text)

    def test_agent_help_hides_legacy_free_code_lab_command(self) -> None:
        parser = build_parser()
        output = io.StringIO()
        ...
        self.assertNotIn("lab-generate", output.getvalue())
