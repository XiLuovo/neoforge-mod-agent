...
> 文档定位：这是 LLM 能力分层补充材料，不是主学习入口。详细机制以 [direct-code-lane.md](direct-code-lane.md) 和 [capability-harvest-loop.md](capability-harvest-loop.md) 为准。
...
Direct Code Lane 仍然不是通用 Coding Agent。它不接受自由 diff，不允许越过生成 workspace，不允许修改工具项目源码，也不会在第一版里自动进行 Direct Code repair-loop。项目不足和后续路线见 [project-limitations.md](project-limitations.md)。
