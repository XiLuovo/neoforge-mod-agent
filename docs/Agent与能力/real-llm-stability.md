...
```powershell
$env:PYTHONPATH = (Resolve-Path .\src)
py -3.11 -m agent.cli real-llm-stability --run-name real-llm-runtime-linked --cases examples\real_llm_stability_cases.json --llm-provider openai-compatible --limit 13 --build --audit --runtime-evidence docs\历史档案\test-matrix\007-2026-05-13-real-llm-natural-prompt-runtime-validation.md --json
```
...
`--require-runtime` 只适合在每个 strict case 都有显式 runtime evidence 时使用；否则报告会失败。这个设计是为了避免把 no-build / no-runtime case 包装成完整 Minecraft runtime 验收。
