...
> 文档定位：这是 Agent 设计取舍专项材料，不是主入口。先看 [architecture.md](../总览/architecture.md)、[agent-workflow.md](../Agent与能力/agent-workflow.md) 和 [project-limitations.md](../总览/project-limitations.md)，再用本文理解为什么采用受控 workflow，而不是开放式 ReAct。
...
Direct Code Lane 是稳定 agent 路径里的“受控补丁通道”：当 ModSpec 表达不足时，允许在生成 workspace 内应用结构化 `write_file` / `replace_text` 计划，并强制 review、snapshot、audit/build 和 rollback evidence。详见 [direct-code-lane.md](../Agent与能力/direct-code-lane.md)。

Free-Code Lab 是实验通道：让 LLM 在隔离 workspace 中探索 generate 覆盖不了的需求，成功后产出 `harvest-candidate.json`，再人工整理成 ModSpec / DSL / generator / audit / tests。详见 [capability-harvest-loop.md](../Agent与能力/capability-harvest-loop.md)。
...
- 后续如果要接 Function Calling / MCP，可以基于 [tool-calling-contract.md](../Agent与能力/tool-calling-contract.md) 包装。
...
- 整体架构：[architecture.md](../总览/architecture.md)
- Agent 阶段真相源：[agent-workflow.md](../Agent与能力/agent-workflow.md)
- Direct Code Lane：[direct-code-lane.md](../Agent与能力/direct-code-lane.md)
- Capability Harvest Loop：[capability-harvest-loop.md](../Agent与能力/capability-harvest-loop.md)
- 工具契约：[tool-calling-contract.md](../Agent与能力/tool-calling-contract.md)
