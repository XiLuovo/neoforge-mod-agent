...
The automated test suite intentionally skips Gradle build by default. Build checks are slower and depend more heavily on the local Java/Gradle environment. They are still covered by smoke commands in `docs/test-matrix.md`.
...
如果要做更强的验证，再运行 `docs/test-matrix.md` 里的 build smoke 命令。
