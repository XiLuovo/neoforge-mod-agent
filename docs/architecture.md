...
更完整的三层解释见 [`docs/llm-capability-layers-cn.md`](llm-capability-layers-cn.md)。
...
Direct Code Lane does not accept free-form diffs, cannot modify this tool project, and cannot touch `.git`, Gradle wrapper jars, build outputs, or paths outside the generated workspace. Known gaps are tracked in [project-limitations.md](project-limitations.md).
...
详细边界见 [capability-harvest-loop.md](capability-harvest-loop.md)。
