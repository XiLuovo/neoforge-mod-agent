...
这份文档面向项目复盘、工程讲解和公开说明。它不替代架构文档，而是帮助把 RC1 主线讲清楚。
...
这个项目从一次性 Minecraft Mod 生成器收束为领域 Coding Agent：LLM 不直接无边界改文件，而是先通过 ModSpec-first 生成 baseline，再在 repair/refine loop 中按结构化 tool action 工作。每个 patch 都限制在 workspace 内并写 snapshot 和 rollback evidence。Reviewer 会做需求覆盖和风险审查，但最终是否成功仍由 deterministic audit/build gate 决定。benchmark 直接运行 develop/repair/reviewer/tool loop，并从 `.agent` trace 统计指标，用来证明 agent 行为可复盘、可量化。
